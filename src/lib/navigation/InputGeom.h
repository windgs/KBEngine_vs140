...
struct BuildSettings
{
	...
	float agentHeight;//代理
	// Agent radius in world units
	float agentRadius;
	// Agent max climb in world units//climb 上升
	float agentMaxClimb;
	// Agent max slope in degrees//slope 斜率
	float agentMaxSlope;
	...
	float edgeMaxLen;
	// Edge max error in voxels//体素中的边最大误差。
	float edgeMaxError;
	float vertsPerPoly;
	// Detail sample distance in voxels//体素中的细节采样距离
	float detailSampleDist;
	// Detail sample max error in voxel heights.//	体素高度中的细节样本最大误差。
	float detailSampleMaxError;
	// Partition type, see SamplePartitionType//分区类型，请参见samplePartitionType
	int partitionType;
