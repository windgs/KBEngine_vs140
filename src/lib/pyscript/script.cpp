...
PyObject * PyTuple_FromStringVector(const std::vector< std::string > & v)
{
	...
	for (int i = 0; i < sz; ++i)
	{
		//add by huyf 2019.08.16:PyTuple_GetItem不会增加引用计数，所以要持有对象，必须Py_XINCREF，Py_Tuple_SetItem会对olditem进行Py_XDECREF
...
int Script::run_simpleString(const char* command, std::string* retBufferPtr)
{
	...
	if(retBufferPtr != NULL)
	{
		...
		v = PyRun_String(command, start, d, d);
...
bool Script::installGameCommon(Network::EventDispatcher* dispatcher)
{
	game_common::installModule(dispatcher);
	return true;
}
