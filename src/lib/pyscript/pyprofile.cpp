...
bool PyProfile::start(std::string profile)
{
	...
	PyObject* pyRet = PyObject_CallFunction(profileMethod_, const_cast<char*>(""));	
	if(!pyRet)
	{
		SCRIPT_ERROR_CHECK();
		return false;
	...
	PyObject* pyRet1 = PyObject_CallMethod(pyRet, const_cast<char*>("enable"), const_cast<char*>(""));
	if(!pyRet1)
	{
		SCRIPT_ERROR_CHECK();
		Py_DECREF(pyRet);
...
bool PyProfile::stop(std::string profile)
{
	...
	PyObject* pyRet = PyObject_CallMethod(iter->second.get(), const_cast<char*>("disable"), const_cast<char*>(""));
	if(!pyRet)
	{
		SCRIPT_ERROR_CHECK();
		return false;
