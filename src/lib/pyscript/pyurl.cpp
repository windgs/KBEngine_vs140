...
PyObject* PyUrl::__py_urlopen(PyObject* self, PyObject* args)
{
	...
	else if (argCount == 3)
	{
		...
		{
			char* s = PyUnicode_AsUTF8AndSize(pyobj, NULL);
			if (s == NULL)
			{
				PyErr_Format(PyExc_AssertionError, "KBEngine::urlopen: args3 is not postData(bytes) or http method!");
				return NULL;
			}
	...
	else if (argCount == 4)
	{
		...
		else if (PyUnicode_Check(pypost))
		{
			char* s = PyUnicode_AsUTF8AndSize(pypost, NULL);
			if (s == NULL)
			{
				PyErr_Format(PyExc_AssertionError, "KBEngine::urlopen: args3 is not postData(bytes) or http method!");
				return NULL;
	...
	if (isPost && postDataLength > 0 && postData)
	{
	...
	{
		Network::Http::Request::Status result = pRequest->setDelete();
		if (Network::Http::Request::OK != result)
			return PyLong_FromLong(result);
	}
	else if (isPut)
	{
		Network::Http::Request::Status result = pRequest->setPut();
		if (Network::Http::Request::OK != result)
			return PyLong_FromLong(result);
	}
