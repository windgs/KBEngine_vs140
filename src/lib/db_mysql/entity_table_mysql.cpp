...
DBID EntityTableMysql::writeTable(DBInterface* pdbi, DBID dbid, int8 shouldAutoLoad, MemoryStream* s, ScriptDefModule* pModule)
{
	...
	DEBUG_MSG(fmt::format("huyf-yh: EntityTableMysql::writeTable-1: parentTableName=[{}], parentTableDBID={}, dbid={}, tableName={}.\n", 
		context.parentTableName, context.parentTableDBID, dbid, context.tableName));
	...
	while(s->length() > 0)
	{
		...
		DEBUG_MSG(fmt::format("huyf-yh: EntityTableMysql::writeTable-2: pid=[{}], child_pid={}.\n", pid, child_pid));
	...
	if(!WriteEntityHelper::writeDB(context.dbid > 0 ? TABLE_OP_UPDATE : TABLE_OP_INSERT, pdbi, context))
		return 0;
...
void EntityTableItemMysql_DIGIT::getWriteSqlItem(DBInterface* pdbi, MemoryStream* s, mysql::DBContext& context)
{
	...
	DEBUG_MSG(fmt::format("huyf-yh: EntityTableItemMysql_DIGIT::getWriteSqlItem: sqlkey=[{}] sqlval=[{}].\n", pSotvs->sqlkey, pSotvs->sqlval));
}
