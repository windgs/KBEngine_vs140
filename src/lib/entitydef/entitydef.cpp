...
// 方法产生时自动产生utype用的
ENTITY_METHOD_UID g_methodUtypeAuto = 30000;
ENTITY_METHOD_UID g_clientMethodUtypeAuto = 1;
std::vector<ENTITY_METHOD_UID> g_methodCusUtypes;																									

ENTITY_PROPERTY_UID g_clientPropertyUtypeAuto = 1;
ENTITY_PROPERTY_UID g_propertyUtypeAuto = 30000;
...
bool EntityDef::finalise(bool isReload)
{
	...
	g_clientMethodUtypeAuto = 1;
	g_methodUtypeAuto = 30000;
	EntityDef::_isInit = false;

	g_clientPropertyUtypeAuto = 1;
	g_propertyUtypeAuto = 30000;
	...
	ScriptDefModule* pScriptModule)
{
	...
	XML_FOR_BEGIN(implementsNode)
	{
		...
		else
		{
			...
			if (!calcDefPropertyUType(moduleName, componentName, (utypeValNode ? defxml->getValInt(utypeValNode) : -1), pScriptModule, flags, futype))
				return false;
...
bool EntityDef::validDefPropertyName(ScriptDefModule* pScriptModule, const std::string& name)
{
	...
	PyObject* pyKBEModule =	PyImport_ImportModule(const_cast<char*>("KBEngine"));

	PyObject* pyEntityModule = PyObject_GetAttrString(pyKBEModule, const_cast<char *>("Entity"));
...
bool EntityDef::calcDefPropertyUType(const std::string& moduleName, 
	const std::string& name, int iUtype, ScriptDefModule* pScriptModule,  uint32 flags, ENTITY_PROPERTY_UID& outUtype)
{
	ENTITY_PROPERTY_UID futype = 0;
	outUtype = futype;
	...
	{
		bool hasClient = ((flags & ENTITY_CLIENT_DATA_FLAGS) > 0);
		while (true)
		{
			if (hasClient)
				futype = g_clientPropertyUtypeAuto++;
			else
	...
	if(defPropertyNode)
	{
		XML_FOR_BEGIN(defPropertyNode)
		{
			...
			if(persistentNode)
			{
				...
				std::transform(strisPersistent.begin(), strisPersistent.end(), strisPersistent.begin(), tolower);
			...
			if(indexTypeNode)
			{
				...
				std::transform(indexType.begin(), indexType.end(), indexType.begin(), toupper);
			}
			...
			if(identifierNode)
			{
				...
				std::transform(strIdentifierNode.begin(), strIdentifierNode.end(), strIdentifierNode.begin(), tolower);
			...
			if (!calcDefPropertyUType(moduleName, name, (utypeValNode ? xml->getValInt(utypeValNode) : -1), pScriptModule, flags, futype))
				return false;
	...
	if(defMethodNode)
	{
		XML_FOR_BEGIN(defMethodNode)
		{
			...
			if(methodDescription->getUType() <= 0)
			{
				...
				while(true)
				{
					if (methodDescription->isClient() || methodDescription->isExposed())
						muid = g_clientMethodUtypeAuto++;
					else
							   ...
							   PyObject* moduleObj, const std::string& moduleName)
{
	...
	PyObject* pyInspectModule = PyImport_ImportModule(const_cast<char*>("inspect"));
	...
	if (pyInspectModule)
	{
		...
		pyGetfullargspec =	PyObject_GetAttrString(pyInspectModule, const_cast<char *>("getfullargspec"));
	}
	else
	{
		SCRIPT_ERROR_CHECK();
			...
			if (pyGetfullargspec)
			{
				...
				else
				{
					...
					else
					{
						...
						// 如果不是这种情况，一律视为参数不正确
						if (iter->second->isExposed() && !g_isReload)
						{
...
//-------------------------------------------------------------------------------------
bool EntityDef::loadAllEntityScriptModules(std::string entitiesPath, std::vector<PyTypeObject*>& scriptBaseTypes)
{
