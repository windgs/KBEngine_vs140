...
public:
	...
	~EntityGarbages()
	{
		DEBUG_MSG(fmt::format("huyf-yh: EntityGarbages<T>::~EntityGarbages!\n"));
	...
	void finalise()
	{
		DEBUG_MSG(fmt::format("huyf-yh: EntityGarbages<T>::finalise!\n"));
		clear();
	}
...
{ 
	...
	else
	{
		...
		if(_lastTime > 0 && timestamp() - _lastTime > uint64(stampsPerSecond()) * 300)//add by huyf 2019.07.08:3600->300
		{
			// 再未清空情况下，下次不提示了
			_lastTime = 0;
	...
	_entities[id] = entity; 
	DEBUG_MSG(fmt::format("huyf-yh: EntityGarbages<T>::add: entityID={} size={}!\n", id, _entities.size()));
}
...
{
	DEBUG_MSG(fmt::format("huyf-yh: EntityGarbages<T>::clear: size={}!\n", _entities.size()));
	_entities.clear();
	_lastTime = 0;
}
...
void EntityGarbages<T>::erase(ENTITY_ID id)
{
	...
	DEBUG_MSG(fmt::format("huyf-yh: EntityGarbages<T>::erase: entityID={} size={}!\n", id, _entities.size()));
}
