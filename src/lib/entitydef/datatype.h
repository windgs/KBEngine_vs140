...
public:	
	...
	virtual bool isSameDefaultValue(PyObject* pyValue, std::string& strDefaultVal) = 0;
	//add end:实体数据序列化优化
...
public:	
	...
	//add by huyf 2019.06.13:实体数据序列化优化
	bool isSameDefaultValue(PyObject* pyValue, std::string& strDefaultVal);
	//add end:实体数据序列化优化
...
//add by huyf 2019.06.13:实体数据序列化优化
template <typename SPECIFY_TYPE>
bool IntType<SPECIFY_TYPE>::isSameDefaultValue(PyObject* pyValue, std::string& strDefaultVal)
{
	if (!IntType<SPECIFY_TYPE>::isSameType(pyValue))
		return false;

	SPECIFY_TYPE val = 0;
	SPECIFY_TYPE defaultVal = 0;
	...
	//DEBUG_MSG(fmt::format("huyf-yh: IntType<SPECIFY_TYPE> defaultVal={} val={} isSame=[{}].\n", defaultVal, val, defaultVal == val ? "=" : "!="));
