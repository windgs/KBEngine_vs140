...
public:	
	...
	*/
	...
	/**
		判断是否等于默认值
	*/	
	virtual bool isSameDefaultValue(PyObject* pyValue);
	//add end:实体数据序列化优化
	...
	*/
	static PropertyDescription* createDescription(ENTITY_PROPERTY_UID utype, 
		const std::string& dataTypeName,	//<Type>          TEAMID          </Type>
		const std::string& name,	// <teamId>
		uint32 flags, //<Flags>         ALL_CLIENTS     </Flags>
		bool isPersistent, //<Persistent>    true            </Persistent>
		DataType* dataType, 
		bool isIdentifier, 
		std::string indexType,
		uint32 databaseLength,
		const std::string& defaultStr,	//<Default>       0               </Default>
		DETAIL_TYPE detailLevel);
