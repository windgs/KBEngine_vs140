...
bool PropertyDescription::isSameDefaultValue(PyObject* pyValue)
{
	if (strcmp(dataTypeName_.c_str(), "permissionGroups") == 0)
	{
		ERROR_MSG(fmt::format("PropertyDescription::isSameDefaultValue: dataTypeName=[{}] defaultValStr_=[{}].\n", dataTypeName_.c_str(), defaultValStr_.c_str()));
	}
	return dataType_->isSameDefaultValue(pyValue, defaultValStr_);
}
//add end:实体数据序列化优化
...
PyObject* PropertyDescription::createFromPersistentStream(MemoryStream* mstream)
{
	...
	//if (strcmp(getName(), "permissionGroups") == 0)
	{
		ERROR_MSG(fmt::format("PropertyDescription::createFromPersistentStream: attrName={} dataTypeName=[{}] defaultValStr_=[{}].\n", getName(), dataTypeName_.c_str(), defaultValStr_.c_str()));
	}
	//add end
	return dataType_->createFromStream(mstream);
}
...
//-------------------------------------------------------------------------------------
PropertyDescription* PropertyDescription::createDescription(ENTITY_PROPERTY_UID utype,	//utype = 30000
															const std::string& dataTypeName,	//dataTypeName = "SPACE_ID"
															const std::string& name,	//name = "space_id"
															uint32 flags,	//flags = 64
															bool isPersistent, 
															DataType* dataType, //dataType.id_ = 4 dataType.aliasName_ = "ACTIVE_ID"
															bool isIdentifier, 
															std::string indexType,
															uint32 databaseLength, 
															const std::string& defaultStr,
															DETAIL_TYPE detailLevel)	//detailLevel = 2
{
...
//  	if (strcmp(name_.c_str(), "compIdCountDict") == 0
// 		|| strcmp(name_.c_str(), "permissionGroups") == 0
// 		|| strcmp(name_.c_str(), "guildDuration") == 0)
//  	{
//  		int n = 0;
//  	}
// 	if (strcmp(dataTypeName_.c_str(), "PYTHON") == 0 
// 		|| strcmp(dataTypeName_.c_str(), "PY_DICT") == 0
// 		|| strcmp(dataTypeName_.c_str(), "PY_LIST") == 0
//  		|| strcmp(dataTypeName_.c_str(), "PY_TUPLE") == 0)
// 	{
// 		DEBUG_MSG(fmt::format("huyf-yh: PropertyDescription::newDefaultVal PropertyDescriptionName={} dataTypeName_={} defaultValStr_={} size={}.\n", name_.c_str(), dataTypeName_.c_str(), defaultValStr_.c_str(), defaultValStr_.size()));
// 	}
	//add end
...
// 		|| strcmp(name_.c_str(), "guildDuration") == 0)
// 	{
// 		DEBUG_MSG(fmt::format("huyf-yh: PropertyDescription::onSetValue name_={} tp_name={}.\n", name_.c_str(), value->ob_type->tp_name));
// 	}
