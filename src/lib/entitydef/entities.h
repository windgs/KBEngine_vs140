...
public:
	...
	void finalise()
	{
		DEBUG_MSG(fmt::format("huyf-yh: Entities<T>::finalise\n"));
		clear(false);
	}
...
{
	DEBUG_MSG(fmt::format("huyf-yh: Entities<T>::clear callScript={}\n", callScript));
...
{
	DEBUG_MSG(fmt::format("huyf-yh: Entities<T>::clear-2 callScript={}\n", callScript));
...
//-------------------------------------------------------------------------------------
//add by huyf 2019.07.08:实体销毁-5
template<typename T>
PyObjectPtr Entities<T>::erase(ENTITY_ID id)
{
	DEBUG_MSG(fmt::format("huyf-yh: Entities<T>::erase: entityID={}!\n", id));
	...
	ERROR_MSG(fmt::format("huyf-yh: Entities<T>::erase: not found {}!\n", id));
	return NULL;
}
