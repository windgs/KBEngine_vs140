...
void ScriptTimers::releaseTimer( TimerHandle handle )
{
	...
	//WARNING_MSG(fmt::format("ScriptTimers::releaseTimer-1: size=({})\n", size));
	//add end
	int numErased = 0;
	...
	while (iter != map_.end())
	{
		...
		if (handle == iter->second)
		{
			map_.erase( iter++ );
			Map::size_type size = map_.size();
			//WARNING_MSG(fmt::format("ScriptTimers::releaseTimer-2: size=({})\n", size));
			++numErased;
		}
		else
		{
...
void ScriptTimers::cancelAll()
{
	Map::size_type size = map_.size();
	//add by huyf
	//WARNING_MSG(fmt::format("ScriptTimers::cancelAll: size=({})\n", size));
	//add end
	...
	for (Map::size_type i = 0; i < size; ++i)
	{
		...
		handle.cancel();
		//add by huyf
		//WARNING_MSG(fmt::format("handle::cancel: size=({}) map_.size()={}\n", size, map_.size()));
		//add end
	}
}
