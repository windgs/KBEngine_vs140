...
void Shutdowner::shutdown(float period, float tickPeriod, Network::EventDispatcher& dispatcher)
{
	...
	pTimerHandle_ = dispatcher.addTimer(int(period * 1000000), this, (void *)TIMEOUT_SHUTDOWN_TICK);
...
void Shutdowner::handleTimeout(TimerHandle handle, void * arg)
{
	switch (reinterpret_cast<uintptr>(arg))
	{
		case TIMEOUT_SHUTDOWN_TICK:
		{
			...
			pTimerHandle_ = pDispatcher_->addTimer(int(tickPeriod_ * 1000000), this, (void *)TIMEOUT_SHUTDOWN_READY_END_TICK);
		...
		case TIMEOUT_SHUTDOWN_READY_END_TICK:
		{
			...
			if (canState != ShutdownHandler::CAN_SHUTDOWN_STATE_TRUE)
			{
				...
				pTimerHandle_ = pDispatcher_->addTimer(int(100000), this, (void *)TIMEOUT_SHUTDOWN_READY_END_TICK);

				break;
			}

			INFO_MSG("Shutdowner::onShutdownEnd: shutting down\n");
			pTimerHandle_ = pDispatcher_->addTimer(int(100000), this, (void *)TIMEOUT_SHUTDOWN_END_TICK);
