...
load_(0.f)
{
	...
	/*
	EntityDef::setGetEntityFunc(std::tr1::bind(&EntityApp<E>::tryGetEntity,  _1,
	std::tr1::placeholders::_2, std::tr1::placeholders::_3));
	tryGetEntity(EntityApp<E>,  参数1, 参数2)
	integer x(0);
...
{
	DEBUG_MSG(fmt::format("huyf-yh: EntityApp<E>::finalise\n"));
...
bool EntityApp<E>::installPyScript()
{
	...
	pyPaths += user_scripts_path + L"common/Lib;";
	...
	getScript().installGameCommon(&this->dispatcher());
	free(tbuf);
	return ret;
}
...
{
	INFO_MSG(fmt::format("huyf-yh: EntityApp<E>::createEntity: entityType={} eid={}\n", entityType, eid));
	...
	if (strcmp(entityType, "CNpc") == 0)
	{
		INFO_MSG(fmt::format("EntityApp::createEntity-before: new entityType={} id={} ob_refcnt={}\n", entityType, id, obj->ob_refcnt));
	}
	...
	else
	{
		if (strcmp(entityType, "CNpc") == 0)
		{
			INFO_MSG(fmt::format("EntityApp::createEntity-after: new entityType={} id={} ob_refcnt={}\n", entityType, id, obj->ob_refcnt));
...
{
	ERROR_MSG(fmt::format("huyf-yh: EntityApp<E>::onSignalled sigNum:{}.\n", sigNum));
...
//add by huyf 2019.07.08:实体销毁-4
//add by huyf 2019.07.08:实体销毁-1
template<class E>
bool EntityApp<E>::destroyEntity(ENTITY_ID entityID, bool callScript)
{
	DEBUG_MSG(fmt::format("huyf-yh: EntityApp::destroyEntity: entityID={}!\n", entityID));
			...
			if(PySequence_Check(path_argsobj))
			{
				...
				for(int i=0; i<size; ++i)
				{
					...
					if(!PyUnicode_Check(pyobj))
					{
						PyErr_Format(PyExc_TypeError, "KBEngine::listPathRes(): args[path, pathargs=\'*.*\'] error!");
						PyErr_PrintEx(0);
						//add by huyf 2019.08.16:这里引用减少引用计数
						//Py_DECREF(pyobj);
						//add end:这里引用减少引用计数
						return 0;
					}
