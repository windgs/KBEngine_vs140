	...
	while(ALL_COMPONENT_TYPES[ifind] != UNKNOWN_COMPONENT_TYPE)
	{
		...
		for(; iter != components.end();)
		{
			if((*iter).pChannel == pChannel)
			{
				...
				{
					_pHandler->onRemoveComponent(componentInfos);
					//add by huyf 2019.07.03:服务器进程崩溃后提示信息优化
					if (componentInfos->componentType == DBMGR_TYPE || componentInfos->componentType == CELLAPPMGR_TYPE || componentInfos->componentType == BASEAPPMGR_TYPE || componentInfos->componentType == BASEAPP_TYPE)
					{
						const int nReportInterval = 2;
						clock_t     tstart, tend;
						int nCpuTime = 0;
						tend = tstart = clock();
__HOOK_REPORT:
						nCpuTime = ((int)(tend - tstart)) / CLOCKS_PER_SEC;
						if (nCpuTime > nReportInterval)
						{
							tstart = clock();
							ERROR_MSG(fmt::format("Components::removeComponentByChannel: {} : {}, Abnormal exit! {}\n",
								COMPONENT_NAME_EX(componentType), (*iter).cid, pChannel->condemnReason()));
