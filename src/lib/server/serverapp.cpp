...
{
	WARNING_MSG(fmt::format("huyf-yh: ServerApp::shutDown:  shutdowntime={}!\n", shutdowntime));
...
void ServerApp::onShutdownBegin()
{
	...
	WARNING_MSG("[INFO]: shutdown begin.\n");
#endif
...
{
	INFO_MSG(fmt::format("huyf-yh: ServerApp::onSignalled sigNum:{}\n", sigNum));
	switch (sigNum)
	{
	case SIGINT:
	case SIGHUP:
