...
Request::Status Request::setDelete()
{
	CURLcode curlCode = curl_easy_setopt((CURL*)pContext_, CURLOPT_CUSTOMREQUEST, "DELETE");
	if (CURLE_OK != curlCode)
	{
		ERROR_MSG(fmt::format("Http::Request::setDelete: "
			"curl_easy_setopt(CURLOPT_CUSTOMREQUEST) error! curlCode={}\n", curlCode));
...
Request::Status Request::setPut()
{
	CURLcode curlCode = curl_easy_setopt((CURL*)pContext_, CURLOPT_PUT, true);
	if (CURLE_OK != curlCode)
	{
		ERROR_MSG(fmt::format("Http::Request::setPut: "
			"curl_easy_setopt(CURLOPT_PUT) error! curlCode={}\n", curlCode));
