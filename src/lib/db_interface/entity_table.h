...
//add by huyf 2019.03.06 任务类图
/*
子类
								class EntityTable（抽象基类：数据库表）-（tableName_：表名称、tableItems_[TABLEITEM_MAP]:所有的字段, pEntityTables_:所涉及到的所有表，tableFixedOrderItems_：和ScriptDefModule中保持一致秩序的item引用、isChild_：子表、sync_）
												v-syncToDB：同步entity表到数据库中
												v-syncIndexToDB：同步entity表索引到数据库中
												v-createItem：创建一个表item
												v-tableItems-tableFixedOrderItems：获得所有表字段
												v-writeTable：更新表
												v-removeEntity：从数据库删除entity
												v-queryTable：获取所有的数据放到流中
												v-entityShouldAutoLoad：设置是否自动加载
												v-queryAutoLoadEntities：查询自动加载的实体

											↓
			——————————————————————————————————————————————————————————————
			↓								↓							↓
			KBETable				EntityTableMysql			EntityTableRedis


							class EntityTableItem（抽象基类：数据库表中的一个字段）：（itemName_：字段名称、tableName_：表的名称、utype_、pParentTable_*、pParentTableItem_*、pDataType_*、pPropertyDescription_*、itemDBType_、datalength_、flags_、indexType_）
											v-syncToDB：同步entity表到数据库中
											v-writeItem：更新数据
											v-queryTable：获取所有的数据放到流中
			...
			————————————————————————————————————————————————————————————————————
			↓																	↓
		EntityTableItemMysqlBase：（db_item_name_、mysqlItemtype_）				EntityTableItemRedisBase
			v-addToStream：获取某个表所有的数据放到流中
			v-getWriteSqlItem
			v-getReadSqlItem：获取需要存储的表名， 字段名和转换为sql存储时的字符串值
			↓
—————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
↓								↓										↓								↓								↓							↓									↓									↓								   ↓										↓
EntityTableItemMysql_DIGIT		EntityTableItemMysql_STRING		EntityTableItemMysql_UNICODE		EntityTableItemMysql_PYTHON		EntityTableItemMysql_BLOB		EntityTableItemMysql_VECTOR2		EntityTableItemMysql_ENTITYCALL		EntityTableItemMysql_ARRAY	   EntityTableItemMysql_FIXED_DICT	    EntityTableItemMysql_Component
