...
{
	DEBUG_MSG(fmt::format("huyf-yh: EntityTable::addItem: tableName_=[{}], itemName_={}, tableName_2={}, utype_={}, itemDBType_={}, datalength_={}, flags_={}.\n", 
		tableName_, pItem->itemName(), pItem->tableName(), pItem->utype(), pItem->itemDBType(), pItem->datalength(), pItem->flags()));
...
DBID EntityTable::writeTable(DBInterface* pdbi, DBID dbid, int8 shouldAutoLoad, MemoryStream* s, ScriptDefModule* pModule)
{
	while(s->length() > 0)
	{
		...
		DEBUG_MSG(fmt::format("huyf-yh: EntityTable::writeTable: item=[{}].\n", pid));
