		...
		for (; iter != propertyDescrs.end(); ++iter)
		{
			...
			else
			{
				...
				if (!propertyDescription->isSameType(pyVal))
				{
					...
					//DEBUG_MSG(fmt::format("Baseapp::createDictDataFromPersistentStream: {}.{} error, set to default!\n", entityType, attrname));
					//add end

					//add by huyf 2019.09.06：不同则取各自属性类型的默认值
					pyVal = propertyDescription->parseDefaultStr("");
					//pyVal = propertyDescription->parseDefaultStr(propertyDescription->getDefaultValStr());
					//add end：不同则取各自属性类型的默认值
				}
				//add by huyf 2019.09.06:解决S_Return(Py_None)覆盖initProperty->createNamespace的问题
				/************************************************************************/
				/* 如果不想区分全部的Persistent属性都入库的话，那么没有这个问题。
				 * 如果base上实体的Persistent属性，如是默认值的话则不想进行入库处理的话。则需要在读取出来该字段的时候会为是NULL，所以重构的时候会创建Py_None对象。
				 * 在进行实体创建的时候initProperty（初始化脚本属性）->createNamespace（用数据库中收集上来的属性字段更新实体属性）这时候
				 * 从数据库中读取出来的是Py_None，该属性会被错误的设置Py_None，导致上层脚本不可以操作此属性的问题。
				/************************************************************************/
// 				if (Py_None == pyVal)
// 				{
// 					Py_DECREF(pyVal);
// 					pyVal = propertyDescription->parseDefaultStr(propertyDescription->getDefaultValStr());
// 				}				
				//add end：解决Py_None覆盖initProperty->createNamespace的问题
			}

			//add by huyf 2019.09.06
// 			if (strcmp(attrname, "permissionGroups") == 0)
// 			{
// 				DEBUG_MSG(fmt::format("Baseapp::createDictDataFromPersistentStream: entityType={} attrname={} tp_name={}\n", entityType, attrname, pyVal->ob_type->tp_name));
// 			}
			//add end
...
//-------------------------------------------------------------------------------------
Baseapp::Baseapp(Network::EventDispatcher& dispatcher, Network::NetworkInterface& ninterface, COMPONENT_TYPE componentType, COMPONENT_ID componentID):
	...
	pBundleImportEntityDefDatas_(NULL)
{
	...
	// hook entitycallcall
	static EntityCallAbstract::EntityCallCallHookFunc entitycallCallHookFunc = std::tr1::bind(&Baseapp::createEntityCallCallEntityRemoteMethod, this, std::tr1::placeholders::_1, std::tr1::placeholders::_2);
...
ShutdownHandler::CAN_SHUTDOWN_STATE Baseapp::canShutdown()
{
	...
	{
		// 所有脚本都加载完毕
		PyObject* pyResult = PyObject_CallMethod(getEntryScript().get(), const_cast<char*>("onReadyForShutDown"), const_cast<char*>(""));
		if (pyResult != NULL)
		{
...
void Baseapp::onShutdown(bool first)
{
	...
	//add by huyf 2019.07.22:cellapp进程全部退出后，才开始销毁baseapp进程中的entity并写入数据库
	Components::COMPONENTS& cellapp_components = Components::getSingleton().getComponents(CELLAPP_TYPE);
	if(cellapp_components.size() == 0)
	{
...
void Baseapp::onGetCreateEntityAnywhereFromDBIDBestBaseappID(Network::Channel* pChannel, KBEngine::MemoryStream& s)
{
	...
	ENTITY_ID entityID = idClient_.alloc();
	KBE_ASSERT(entityID > 0);
	//add by huyf 2019.09.06:模式对应DBTaskQueryEntity 类中的 0 1 2 
	int queryMode = 1;	
	//add end
	DbmgrInterface::queryEntityArgs7::staticAddToBundle((*pBundle),
		dbInterfaceIndex, targetComponentID, queryMode, dbid, entityType, callbackID, entityID);
...
{
	DEBUG_MSG(fmt::format("huyf-yh: Baseapp::createCellEntity: scriptName=[{}] id=[{}] \n", pEntity->scriptName(), pEntity->id()));
...
void Baseapp::onCellWriteToDBCompleted(Network::Channel* pChannel, KBEngine::MemoryStream& s)
{
	...
	if(pEntity)
	{
		...
		INFO_MSG(fmt::format("huyf-yh: Baseapp::onCellWriteToDBCompleted: scriptName={} entityID=({}).\n",
						...
						const std::string& encryptedKey)
{
	...
	uint64 now = static_cast<uint64>(::time(NULL));
	(*pBundle) << now;
