...
//-------------------------------------------------------------------------------------
void CoordinateSystem::moveNodeX(CoordinateNode* pNode, float px, CoordinateNode* pCurrNode, bool equalMove)
{
	if(pCurrNode != NULL)
	{
		bool pNodeIsNext = (pCurrNode->pNextX() == pNode);
		if(pCurrNode->x() > px || (pCurrNode->x()==px && pNodeIsNext && equalMove))
		{
...
//-------------------------------------------------------------------------------------
void CoordinateSystem::moveNodeY(CoordinateNode* pNode, float py, CoordinateNode* pCurrNode, bool equalMove)
{
	if(pCurrNode != NULL)
	{
		bool pNodeIsNext = (pCurrNode->pNextY() == pNode);
		if(pCurrNode->y() > py || (pCurrNode->y() == py && pNodeIsNext && equalMove))
		{
...
//-------------------------------------------------------------------------------------
void CoordinateSystem::moveNodeZ(CoordinateNode* pNode, float pz, CoordinateNode* pCurrNode, bool equalMove)
{
	if(pCurrNode != NULL)
	{
		bool pNodeIsNext = (pCurrNode->pNextZ() == pNode);
		if(pCurrNode->z() > pz || (pCurrNode->z() == pz && pNodeIsNext && equalMove))
		{
...
void CoordinateSystem::update(CoordinateNode* pNode)
{
	...
	if(pNode->xx() != pNode->old_xx())
	{
		while(true)
		{
			...
			bool isEqualMove = false;
			//由于COORDINATE_NODE_FLAG_BOUNDARIES计算时边界算在内部，所以prevX和它相等时也做一次移动，保证触发onNodePassX，继而触发onEnter进入view，使两边一致
			while(pCurrNode && pCurrNode != pNode && (pCurrNode->x() > pNode->xx() || (pCurrNode->x() == pNode->xx() && pNode->hasFlags(COORDINATE_NODE_FLAG_BOUNDARIES))))
			{
				pNode->x(pCurrNode->x());
				...
				#endif
				//记录这一次移动是不是等于时的移动，避免后面处理next时再把它移回来
				isEqualMove = (pCurrNode->x() == pNode->xx());
				// 先把节点移动过去
				moveNodeX(pNode, pNode->xx(), pCurrNode, isEqualMove);
			...
			//前面没移动过这里才移动，避免来回动
			while (pCurrNode && pCurrNode != pNode && (pCurrNode->x() < pNode->xx() || (!isEqualMove && pCurrNode->x() == pNode->xx() && pNode->hasFlags(COORDINATE_NODE_FLAG_BOUNDARIES))))
			{
				pNode->x(pCurrNode->x());
				...
				// 先把节点移动过去
				moveNodeX(pNode, pNode->xx(), pCurrNode, pCurrNode->x() == pNode->xx());
	...
	if(CoordinateSystem::hasY && pNode->yy() != pNode->old_yy())
	{
		while(true)
		{
			...
			while(pCurrNode && pCurrNode != pNode && pCurrNode->y() > pNode->yy())
			{
				...
				// 先把节点移动过去
				moveNodeY(pNode, pNode->yy(), pCurrNode, false);
	...
	if(pNode->zz() != pNode->old_zz())
	{
		while(true)
		{
			...
			bool isEqualMove = false;
			while(pCurrNode && pCurrNode != pNode && (pCurrNode->z() > pNode->zz() || (pCurrNode->z() == pNode->zz() && pNode->hasFlags(COORDINATE_NODE_FLAG_BOUNDARIES))))
			{
				pNode->z(pCurrNode->z());
				...
				#endif
				isEqualMove = (pCurrNode->z() == pNode->zz());
				// 先把节点移动过去
				moveNodeZ(pNode, pNode->zz(), pCurrNode, isEqualMove);
			...
			while(pCurrNode && pCurrNode != pNode && (pCurrNode->z() < pNode->zz() || (!isEqualMove && pCurrNode->z() == pNode->zz() && pNode->hasFlags(COORDINATE_NODE_FLAG_BOUNDARIES))))
			{
				pNode->z(pCurrNode->z());
				...
				// 先把节点移动过去
				moveNodeZ(pNode, pNode->zz(), pCurrNode, pCurrNode->z() == pNode->zz());
