...
SCRIPT_METHOD_DECLARE("hasEntity",					pyCheckHasEntity,				METH_VARARGS,				0)
...
SCRIPT_GETSET_DECLARE("collidable",					pyGetCollidable,				pySetCollidable,			0,		0)
...
pCustomVolatileinfo_(NULL)
{
	...
	{
		int nRefCount = ((PyObject*)this)->ob_refcnt;
		DEBUG_MSG(fmt::format("huyf-yh: Entity::Entity p={:p} ScriptModule={} id_={} nRefCount={}\n", (void*)this, pScriptModule_->getName(), id_, nRefCount));
	}	
}
...
Entity::~Entity()
{
	DEBUG_MSG(fmt::format("huyf-yh: Entity::~Entity-0 ScriptModule={} id_={}\n", pScriptModule_->getName(), id_));
...
//-------------------------------------------------------------------------------------
//add by huyf 2019.07.08:实体销毁-7
void Entity::onDestroy(bool callScript)
{
	DEBUG_MSG(fmt::format("huyf-yh: Entity::onDestroy callScript={}\n", callScript));

	if(callScript && isReal())
	{
		//add by huyf 2019.07.08:如果这个函数在脚本中有实现，这个函数在调用Entity.destroy()后，在实际销毁之前被调用。这个函数没有参数。
		...
		if(baseEntityCall_ != NULL)
		{
			...
			DEBUG_MSG(fmt::format("huyf-yh: Entity::onDestroy-2 BaseappInterface::onLoseCell\n"));
		}
		DEBUG_MSG(fmt::format("huyf-yh: Entity::onDestroy-3 baseEntityCall_ == NULL\n"));
	}
...
PyObject* Entity::__py_pyDestroyEntity(PyObject* self, PyObject* args, PyObject * kwargs)
{
	...
	if (strcmp(pobj->scriptName(), "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("CellApp::{}::__py_pyDestroyEntity-before: id={} ob_refcnt={}.\n", pobj->scriptName(), pobj->id(), ((PyObject *)(pobj))->ob_refcnt));
	}
	...
	pobj->destroyEntity();
	DEBUG_MSG(fmt::format("CellApp::{}::__py_pyDestroyEntity-after: id={} ob_refcnt={}.\n", pobj->scriptName(), pobj->id(), ((PyObject *)(pobj))->ob_refcnt));
...
PyObject* Entity::pyGetCollidable()
{
	if (collidable_)
		Py_RETURN_TRUE;
	Py_RETURN_FALSE;
...
//-------------------------------------------------------------------------------------
int Entity::pySetCollidable(PyObject *value)
{
	collidable_ = PyObject_IsTrue(value);
	return 0;
}
...
//-------------------------------------------------------------------------------------
void Entity::onRemoteMethodCall_(PropertyDescription* pComponentPropertyDescription, MethodDescription* pMethodDescription, ENTITY_ID srcEntityID, MemoryStream& s)
{
	SCOPED_PROFILE(SCRIPTCALL_PROFILE);
	...
	if (pComponentPropertyDescription)
	{
		pyCallObject = PyObject_GetAttrString(this, const_cast<char*>(pComponentPropertyDescription->getName()));
	}
...
/*
* 1、用于cell进程中的实体属性汇报给base进程的时候调用：backupCellData
* 2、
*/
void Entity::addCellDataToStream(COMPONENT_TYPE sendTo, uint32 flags, MemoryStream* mstream, bool useAliasID)
{
	...
	INFO_MSG(fmt::format("huyf-yh: Entity::addCellDataToStream: scriptName={} entityID=({}).\n",
	...
	for(; iter != propertyDescrs.end(); ++iter)
	{
		...
		if((flags & propertyDescription->getFlags()) > 0)
		{
			...
			//add by huyf
			if ((strcmp(propertyDescription->getName(), "gameEntityId") == 0|| strcmp(propertyDescription->getName(), "gbId") == 0) 
				&& (strcmp(this->scriptName(), "Collection") == 0 || strcmp(this->scriptName(), "Avatar") == 0))
			{
				int i = 0;
			}
			//add
...
{
	//add by huyf 2019.06.20
	INFO_MSG(fmt::format("huyf-yh: Entity::backupCellData: scriptName={} entityID=({}).\n",
		this->scriptName(), this->id()));
	//add end
...
{
	//add by huyf 2019.06.20
	INFO_MSG(fmt::format("huyf-yh: Cellapp::writeToDB: scriptName={} entityID=({}).\n",
		this->scriptName(), this->id()));
	//add end
...
bool Entity::bufferOrExeCallback(const char * funcName, PyObject * funcArgs, bool notFoundIsOK)
{
	...
	{
		if ((((PyObject *)this)->ob_type->tp_name, "CNpc") == 0)
		{
			DEBUG_MSG(fmt::format("huyf-yh: Entity::bufferOrExeCallback-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)this, scriptName(), id(), ((PyObject *)this)->ob_refcnt));
		}
		...
		Py_DECREF(this);
		//DEBUG_MSG(fmt::format("huyf-yh: Entity::bufferOrExeCallback-Py_DECREF scriptName={} id={} ob_refcnt = {}\n", scriptName(), id(), ((PyObject *)this)->ob_refcnt));
	}
...
PyObject* Entity::pyCheckHasEntity(int x, int z)
{
	if (this->pEntityCoordinateNode()->hasEntity(x, z))
	{
		Py_RETURN_TRUE;
	}
	Py_RETURN_FALSE;
...
void Entity::onGetWitness(bool fromBase)
{
	...
	if ((((PyObject *)this)->ob_type->tp_name, "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("huyf-yh: Entity::onGetWitness-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)this, scriptName(), id(), ((PyObject *)this)->ob_refcnt));
	}
...
{	
	...
	if (pControllers())
		controllerSize = pControllers()->objects().size();
	PyObject* pyList = PyList_New(timerMap.size()+ controllerSize);
	int i = 0;
	uint32 time;
	uint32 interval;
	void* pUser;
	...
	{	
		Cellapp::getSingleton().timers().getTimerInfo(it->second, time, interval, pUser);
	...
	for (KBEngine::Controllers::CONTROLLERS_MAP::const_iterator it = pControllers()->objects().begin();it != pControllers()->objects().end();it++)
	{
		std::string controllerType = "unknown";
		switch (it->second->type())
		{
		case KBEngine::Controller::CONTROLLER_TYPE_MOVE:
			controllerType = "move";
			break;
		case KBEngine::Controller::CONTROLLER_TYPE_NORMAL:
			controllerType = "normal";
			break;
		case KBEngine::Controller::CONTROLLER_TYPE_PROXIMITY:
			controllerType = "proximity";
			break;
		case KBEngine::Controller::CONTROLLER_TYPE_ROTATE:
			controllerType = "rotate";
			break;
		default:
			break;
		}
		PyObject* pyTup = PyTuple_New(3);
		PyTuple_SetItem(pyTup, 0, PyUnicode_FromString(controllerType.c_str()));
		PyTuple_SetItem(pyTup, 1, PyLong_FromLong(it->first));
		PyTuple_SetItem(pyTup, 2, PyLong_FromLong(it->second->userarg()));
		PyList_SET_ITEM(pyList, i++, pyTup);
	}
	return pyList;
}
...
bool Entity::navigatePathPoints( std::vector<Position3D>& outPaths, const Position3D& destination, float maxSearchDistance, int8 layer )
{
    ...
    if (pSpace->findStraightPath(pEntityCoordinateNode(), layer, position_, destination, outPaths) < 0)
    {
        WARNING_MSG(fmt::format("Entity::navigatePathPoints(): space({}), entityID({}), find path error!\n",
            spaceID(), id()));
        return false;
    }
	...
	// 第一个坐标点是当前位置，因此可以过滤掉
	if (pNavHandle->type()!=pNavHandle->NAV_TILE && iter != outPaths.begin())
	{
...
PyObject* Entity::pyNavigatePathPoints(PyObject_ptr pyDestination, float maxSearchDistance, int8 layer)
{
	...
	for(; iter != outPaths.end(); ++iter)
	{
		...
		//add by huyf 2019.08.16:这里不应该在增加引用计数了
		Py_INCREF(pos);
		//add end:这里不应该在增加引用计数了
	...
	float maxRadius, uint32 maxPoints, int8 layer)
{
    ...
    return pSpace->findRandomPointAroundCircle(pEntityCoordinateNode(), layer, centerPos, outPoints, maxPoints, maxRadius) > 0;
}
...
PyObject* Entity::pyEntitiesInView()
{
	...
	for(; iter != pWitness_->viewEntities().end(); ++iter)
	{
		...
		if(pEntity)
		{
			...
			{
				DEBUG_MSG(fmt::format("huyf-yh: Entity::pyEntitiesInView-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity, pEntity->scriptName(), pEntity->id(), ((PyObject *)pEntity)->ob_refcnt));
			}
		}
	}
...
PyObject* Entity::__py_pyEntitiesInRange(PyObject* self, PyObject* args)
{
	...
	for (; iter != findentities.end(); ++iter)
	{
		...
		if ((((PyObject *)pEntity)->ob_type->tp_name, "CNpc") == 0)
		{
			DEBUG_MSG(fmt::format("huyf-yh: Entity::__py_pyEntitiesInRange-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity, pEntity->scriptName(), pEntity->id(), ((PyObject *)pEntity)->ob_refcnt));
		}
	}
...
void Entity::teleport(PyObject_ptr nearbyMBRef, Position3D& pos, Direction3D& dir)
{
	...
	*/
	...
	if ((((PyObject *)this)->ob_type->tp_name, "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("huyf-yh: Entity::teleport-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)this, scriptName(), id(), ((PyObject *)this)->ob_refcnt));
	}
	Py_INCREF(this);
...
bool Entity::hasNeighborEntity(int dx, int dz)
{
	CoordinateNode* pRootNode = this->pEntityCoordinateNode();
	int xfrom, xto, zfrom, zto;
	int floorx = static_cast<int>(floor(pRootNode->x()));
	int floorz = static_cast<int>(floor(pRootNode->z()));
	if (dx >0)
	{
		xfrom = floorx + 1;
		xto = xfrom + dx;
	...
	for (int i = xfrom; i < xto; i++)
		for (int j = zfrom; j < zto; j++)
			if (this->pEntityCoordinateNode()->hasEntity(i, j))
				return true;
	return false;

	/*CoordinateNode* pCurr=pRootNode;
	...
	if (dx!=0)
	{
		do
		{
			...
			if (pCurr && !pCurr->hasFlags(COORDINATE_NODE_FLAG_HIDE_OR_REMOVED) && pCurr->hasFlags(COORDINATE_NODE_FLAG_ENTITY))
			{
				Entity* pEntity = static_cast<EntityCoordinateNode*>(pCurr)->pEntity();

				const Position3D& pos = pEntity->position();
				if ((dx<0 && pos.x >= xto) || (dx > 0 && pos.x <= xfrom))
					continue;
				else if ((dx < 0 && pos.x < xfrom) || (dx > 0 && pos.x > xto))
					break;

				if (pos.z > zfrom && pos.z < zto)
					return true;
			}
			else if (pCurr && !pCurr->hasFlags(COORDINATE_NODE_FLAG_HIDE_OR_REMOVED))
			{
				if ((dx < 0 && pCurr->x() <= xfrom) || (dx > 0 && pCurr->x() >= xto))
					break;
			}
		}while(pCurr);
	}
	else if (dz != 0)
	{
		do
		{
			...
			if (pCurr && !pCurr->hasFlags(COORDINATE_NODE_FLAG_HIDE_OR_REMOVED) && pCurr->hasFlags(COORDINATE_NODE_FLAG_ENTITY))
			{
				...
				const Position3D& pos = pEntity->position();
				if ((dz<0 && pos.z >= zto) || (dz > 0 && pos.z <= zfrom))
					continue;
				else if ((dz < 0 && pos.z <= zfrom) || (dz > 0 && pos.z >= zto))
					break;

				if (pos.x > xfrom && pos.x < xto)
					return true;
			}
			else if (pCurr && !pCurr->hasFlags(COORDINATE_NODE_FLAG_HIDE_OR_REMOVED))
			{
				if((dz<0 && pCurr->z() > zto) || (dz > 0 && pCurr->z() < zfrom))
					continue;
				else if ((dz < 0 && pCurr->z() < zfrom) || (dz > 0 && pCurr->z() > zto))
					break;
			}
		}while(pCurr);
	}
...
{																										
	...
	{																									
		...
		/*DEBUG_MSG(fmt::format(#CLASS" huyf-yh: PyDict_Next pos=[{}] key=[{}].\n", pos, ccattrTp));*/	
