...
public:
	...
	*/
	...
	DECLARE_PY_MOTHOD_ARG0(pyGetControllers);
	...
	DECLARE_PY_MOTHOD_ARG2(pyCheckHasEntity, int, int);
	DECLARE_PY_MOTHOD_ARG1(pySetCollidable, int);
	...
	*/
	...
	INLINE bool collidable() const { return collidable_; };
	DECLARE_PY_GETSET_MOTHOD(pyGetCollidable, pySetCollidable);
	
	/** 
		脚本请求获得一定范围内的某种类型的entities 
	*/
	static PyObject* __py_pyEntitiesInRange(PyObject* self, PyObject* args);
	bool hasNeighborEntity(int dx, int dz);
	/** 
