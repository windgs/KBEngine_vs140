...
entityNodeUpdating_(0)
{
	...
	Py_INCREF(pEntity_);
	if ((((PyObject *)pEntity_)->ob_type->tp_name, "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("huyf-yh: Entity::EntityCoordinateNode-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity_, pEntity_->scriptName(), pEntity_->id(), ((PyObject *)pEntity_)->ob_refcnt));
	}
}
...
EntityCoordinateNode::~EntityCoordinateNode()
{
	...
	if ((((PyObject *)pEntity_)->ob_type->tp_name, "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("huyf-yh: Entity::~EntityCoordinateNode-Py_DECREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity_, pEntity_->scriptName(), pEntity_->id(), ((PyObject *)pEntity_)->ob_refcnt));
		printf("1 [%s(%d)] _tp_dealloc %s op=%p ob_refcnt=%d\n", TrimFilePath(__FILE__), __LINE__, ((PyObject *)(pEntity_))->ob_type->tp_name, (void*)(pEntity_), ((PyObject *)(pEntity_))->ob_refcnt);
	}
}
...
bool EntityCoordinateNode::hasEntity(int x, int z)
{
	CoordinateNode* pCoordinateNode = this;
	Position3D pos = Position3D(x, 0, z);
	CoordinateNodeWrapX wrapx(pCoordinateNode, pos);
	KBEngine::ENTITY_ID callerId = wrapx.currentNodeEntity()->id();
	Position3D curPos = wrapx.currentNodeEntity()->position();
	KBEngine::ENTITY_ID curId = wrapx.currentNodeEntity()->id();
	bool curCollidable = wrapx.currentNodeEntity()->collidable();

	if (static_cast<int>(floor(curPos.x)) == x && static_cast<int>(floor(curPos.z)) == z)
		return false;

	while (floor(curPos.x) >= x)
	{
		if (curId != callerId && curCollidable && static_cast<int>(floor(curPos.x)) == x && static_cast<int>(floor(curPos.z)) == z)
			return true;
		if (!wrapx.prev())
			break;
		if (wrapx.isEntityNode())
		{
			Position3D prevPos = wrapx.currentNodeEntity()->position();
			curId = wrapx.currentNodeEntity()->id();
			curPos.x = prevPos.x;
			curPos.y = prevPos.y;
			curPos.z = prevPos.z;
			curCollidable = wrapx.currentNodeEntity()->collidable();
		}
		else
		{
			curPos.x = wrapx.currentNode()->x();
			curPos.y = wrapx.currentNode()->y();
			curPos.z = wrapx.currentNode()->z();
		}
	};
		
	wrapx.reset();
	curPos = wrapx.currentNodeEntity()->position();
	curId = wrapx.currentNodeEntity()->id();
	curCollidable = wrapx.currentNodeEntity()->collidable();
	...
	while (floor(curPos.x) <= x)
	{
		...
		if (wrapx.isEntityNode())
		{
			Position3D nextPos = wrapx.currentNodeEntity()->position();
			curId = wrapx.currentNodeEntity()->id();
			curPos.x = nextPos.x;
			curPos.y = nextPos.y;
			curPos.z = nextPos.z;
