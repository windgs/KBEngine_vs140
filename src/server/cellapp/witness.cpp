...
void Witness::onEnterView(ViewTrigger* pViewTrigger, Entity* pEntity)
{
	...
	{
		DEBUG_MSG(fmt::format("huyf-yh: Witness::onEnterView-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity, pEntity->scriptName(), pEntity->id(), ((PyObject *)pEntity)->ob_refcnt));
	}
	...
	if ((((PyObject *)pSelfEntity)->ob_type->tp_name, "CNpc") == 0)
	{
		DEBUG_MSG(fmt::format("huyf-yh: Witness::onEnterView-Py_INCREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pSelfEntity, pSelfEntity->scriptName(), pSelfEntity->id(), ((PyObject *)pSelfEntity)->ob_refcnt));
	}
	...
	if (iter != viewEntities_map_.end())
	{
		...
		if ((((PyObject *)pEntity)->ob_type->tp_name, "CNpc") == 0)
		{
			DEBUG_MSG(fmt::format("huyf-yh: Witness::onEnterView-Py_DECREF p={:p} scriptName={} id={} ob_refcnt = {}\n", (void*)pEntity, pEntity->scriptName(), pEntity->id(), ((PyObject *)pEntity)->ob_refcnt));
		}
	...
	case UPDATE_FLAG_XYZ:
		{
			...
			//printf("witness_xyz1---------------------------, %f, %f\n", otherEntity->position().x, otherEntity->position().z);
			//printf("witness_xyz2---------------------------, %f, %f\n", this->pEntity()->position().x, this->pEntity()->position().z);
			//printf("witness_xyz3---------------------------, %f, %f\n", relativePos.x, relativePos.z);
