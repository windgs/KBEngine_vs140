...
bool ClientSDK::generateCommonMap(const EntityDef::SCRIPT_MODULES& modules)
{
	EntityDef::SCRIPT_MODULES::const_iterator moduleIter = modules.begin();
	for (; moduleIter != modules.end(); ++moduleIter)
	{
		...
		for (; propIter != clientPropertys.end(); ++propIter)
		{
			...
			std::string propDescKey = CommonPropDesc::getPropDescKey(propIter->first, pPropertyDescription->getDataTypeName());
			if (propsMap_.find(propDescKey) != propsMap_.end())
				propsMap_[propDescKey].addDefinedEnt(pScriptDefModule->getName());
			else
				propsMap_[propDescKey] = CommonPropDesc(propIter->first, pPropertyDescription, pScriptDefModule->getName());
		}

		ScriptDefModule::METHODDESCRIPTION_MAP& clientMethods = pScriptDefModule->getClientMethodDescriptions();
		...
		for (; methodIter != clientMethods.end(); ++methodIter)
		{
			...
			std::string descKey = CommonMethodDesc::getMethodDescKey(methodIter->first, argTypes);

			if (methodsMap_.find(descKey) != methodsMap_.end())
			{
				methodsMap_[descKey].addDefinedEnt(pScriptDefModule->getName());
			}
			else
			{
				methodsMap_[descKey] = CommonMethodDesc(methodIter->first, pScriptDefModule->getName(), pMethodDescription);
				for (size_t i = 0;i < argTypes.size();i++)
					methodsMap_[descKey].addArg(argTypes[i]->getName());
			}
		}
	}
	return true;
...
bool ClientSDK::needClientMethod(MethodDescription* pMethodDescription)
{
	std::string methodName = pMethodDescription->getName();
	std::vector<DataType*>& argTypes = pMethodDescription->getArgTypes();
	std::string descKey = CommonMethodDesc::getMethodDescKey(methodName, argTypes);
	KBE_ASSERT(methodsMap_.find(descKey) != methodsMap_.end());

	if (methodsMap_[descKey].definedByEnts.size() > 1)
		return false;
	return true;
}
	...
	ScriptDefModule* pCurrScriptDefModule)
{
	...
	{
		MethodDescription* pMethodDescription = methodIter->second;
		if (!needClientMethod(pMethodDescription))
			continue;
