...
bool ClientSDKUnity::writeEntityModuleBegin(ScriptDefModule* pEntityScriptDefModule)
{
	...
	else
	{
		...
		sourcefileBody_ += fmt::format("\tpublic abstract class {} : ModelEntity\n\t{{\n", newModuleName);
...
bool ClientSDKUnity::writeEntityProcessMessagesMethod(ScriptDefModule* pEntityScriptDefModule)
{
	...
	if (!pEntityScriptDefModule->isComponentModule())
	{
		...
		sourcefileBody_ += fmt::format("\t\t\tbool useMethodDescrAlias = sm.useMethodDescrAlias;\n\n");
		...
		if (foundComponentNoUseMethodDescrAlias)
		{
			...
			sourcefileBody_ += fmt::format("\t\t\t\t\t{{\n");
			sourcefileBody_ += fmt::format("\t\t\t\t\t\tuseMethodDescrAlias = false;\n");
			sourcefileBody_ += fmt::format("\t\t\t\t\t}}\n");
		...
		sourcefileBody_ += fmt::format("\t\t\t}}\n\n");
		sourcefileBody_ += fmt::format("\t\t\tif(useMethodDescrAlias)\n");
		sourcefileBody_ += fmt::format("\t\t\t{{\n");
...
bool ClientSDKUnity::needOnPropChangedMethod(PropertyDescription* pPropertyDescription)
{
	std::string propName = pPropertyDescription->getName();
	std::string propDescKey = CommonPropDesc::getPropDescKey(propName, pPropertyDescription->getDataTypeName());
	KBE_ASSERT(propsMap_.find(propDescKey) != propsMap_.end());

	if (propsMap_[propDescKey].definedByEnts.size() > 1)
	{
		return false;
	}
	return true;
	...
	ScriptDefModule* pCurrScriptDefModule, PropertyDescription* pPropertyDescription)
{
	...
	if (needOnPropChangedMethod(pPropertyDescription))
	{
		std::string name = pPropertyDescription->getName();
		name[0] = std::toupper(name[0]);
...
{
	//clientMethod
	sourcefileBody_ += fmt::format("\t\tpublic virtual void {}({}) {{}} \n", pMethodDescription->getName(), fillString);
	return true;
}
...
void ClientSDKUnity::saveEntitCommonFile()
{
	...
	std::string path = currSourcePath_ + "EntityCommon.cs";

	DEBUG_MSG(fmt::format("ClientSDK::saveEntitCommonFile(): {}\n", path).c_str());

	FILE *fp = fopen(path.c_str(), "w");

	if (NULL == fp)
	{
		ERROR_MSG(fmt::format("ClientSDK::saveEntitCommonFile(): fopen error! {}\n", path));

		return;
	}

	int written = fwrite(entityCommonBody_.c_str(), 1, entityCommonBody_.size(), fp);
	if (written != (int)entityCommonBody_.size())
	{
		ERROR_MSG(fmt::format("ClientSDK::saveEntitCommonFile(): fwrite error! {}\n", path));
		return;
	}

	if (fclose(fp))
	{
		ERROR_MSG(fmt::format("ClientSDK::saveEntitCommonFile(): fclose error! {}\n",	path));

		return;
	}
}

bool ClientSDKUnity::writeEntityCommonClass()
{

	entityCommonBody_ = "";

	entityCommonBody_ += "namespace KBEngine\n{\n";
	entityCommonBody_ += "\tusing UnityEngine;\n";
	entityCommonBody_ += "\tusing System;\n";
	entityCommonBody_ += "\tusing System.Collections;\n";
	entityCommonBody_ += "\tusing System.Collections.Generic;\n\n";

	entityCommonBody_ += fmt::format("\tpublic abstract class {} : Entity\n\t{{\n", "EntityCommon");

	std::map<std::string, CommonMethodDesc>::iterator mdIter = methodsMap_.begin();
	for (; mdIter != methodsMap_.end(); mdIter++)
	{
		CommonMethodDesc& methodDesc = mdIter->second;
		if (methodDesc.definedByEnts.size() <= 1)
			continue;
		MethodDescription* pMethodDescription = methodDesc.pMethodDesc;

		std::string comment = "";
		for (size_t i = 0;i < methodDesc.definedByEnts.size();i++)
			comment += methodDesc.definedByEnts[i]+ " ";
		//clientMethod
		std::string methodString = fmt::format("\t\tpublic virtual void {}({}) {{}} //{}\n", pMethodDescription->getName(), "#REPLACE#", comment);
		std::string argsBody = "";

		std::vector<DataType*>& argTypes = pMethodDescription->getArgTypes();
		std::vector<DataType*>::const_iterator iter = argTypes.begin();
		...
		strutil::kbe_replace(methodString, "#REPLACE#", argsBody);
		entityCommonBody_ += methodString;
	}

	entityCommonBody_ += "\n\n";

	std::map<std::string, CommonPropDesc>::iterator ppIter = propsMap_.begin();
	for (; ppIter != propsMap_.end(); ppIter++)
	{
		CommonPropDesc& propDesc = ppIter->second;
		if (propDesc.definedByEnts.size() <= 1)
			continue;

		std::string comment = "";
		for (size_t i = 0;i < propDesc.definedByEnts.size();i++)
			comment += propDesc.definedByEnts[i] + " ";

		std::string name = propDesc.pPropDesc->getName();
		name[0] = std::toupper(name[0]);
		std::string type = propDesc.pPropDesc->getDataType()->getName();
		if (type == "INT8")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(SByte oldValue) {{}}", name);
		}
		else if (type == "INT16")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Int16 oldValue) {{}}", name);
		}
		else if (type == "INT32")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Int32 oldValue) {{}}", name);
		}
		else if (type == "INT64")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Int64 oldValue) {{}}", name);
		}
		else if (type == "UINT8")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Byte oldValue) {{}}", name);
		}
		else if (type == "UINT16")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(UInt16 oldValue) {{}}", name);
		}
		else if (type == "UINT32")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(UInt32 oldValue) {{}}", name);
		}
		else if (type == "UINT64")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(UInt64 oldValue) {{}}", name);
		}
		else if (type == "FLOAT")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(float oldValue) {{}}", name);
		}
		else if (type == "DOUBLE")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(double oldValue) {{}}", name);
		}
		else if (type == "STRING")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(string oldValue) {{}}", name);
		}
		else if (type == "UNICODE")
		{
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(string oldValue) {{}}", name);
		...
		{
			if (std::string("ARRAY") == propDesc.pPropDesc->getDataTypeName())
			{
				std::string typeStr;
				std::string childItemName = propDesc.pPropDesc->getDataType()->aliasName();
				if (childItemName.size() == 0 || childItemName[0] == '_')
				{
					getArrayType(propDesc.pPropDesc->getDataType(), typeStr);
				}
				else
				{
					typeStr = childItemName;
				...
				entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed({} oldValue) {{}}", name, typeStr);
			}
			else
			{
				entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed({} oldValue) {{}}", name, propDesc.pPropDesc->getDataTypeName());
			}
		}
		else if (type == "FIXED_DICT")
		{
		...
		{
#ifdef CLIENT_NO_FLOAT
			sourcefileBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector2Int oldValue) {{}}", name);
#else
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector2 oldValue) {{}}", name);
#endif
		}
		else if (type == "VECTOR3")
		{
#ifdef CLIENT_NO_FLOAT
			sourcefileBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector3Int oldValue) {{}}", name);
#else
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector3 oldValue) {{}}", name);
#endif
		}
		else if (type == "VECTOR4")
		{
#ifdef CLIENT_NO_FLOAT
			sourcefileBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector4Int oldValue) {{}}", name);
#else
			entityCommonBody_ += fmt::format("\t\tpublic virtual void on{}Changed(Vector4 oldValue) {{}}", name);
#endif
		}
		else if (type == "ENTITYCALL")
		{
