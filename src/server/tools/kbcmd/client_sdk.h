...
class CommonPropDesc
{
public:
	std::string propName;
	const PropertyDescription* pPropDesc;
	std::vector <std::string> definedByEnts;

public:
	CommonPropDesc() : propName(), pPropDesc(NULL), definedByEnts() { };
	CommonPropDesc(std::string propName, const PropertyDescription* pPD, std::string definedEnt) : propName(propName), pPropDesc(pPD), definedByEnts() { definedByEnts.push_back(definedEnt); };
	void addDefinedEnt(std::string definedEnt) { definedByEnts.push_back(definedEnt); };
	static std::string getPropDescKey(std::string propName, std::string propType) { return propName + "("+propType+")"; };
};
...
public:
	...
	std::vector<std::string> definedByEnts;
	MethodDescription* pMethodDesc;

public:
	CommonMethodDesc() : methodName(), argTypes(), definedByEnts(), pMethodDesc(NULL) { };
	CommonMethodDesc(std::string methodName, std::string definedEnt, MethodDescription* pMD) : methodName(methodName), argTypes(), definedByEnts(), pMethodDesc(pMD) { definedByEnts.push_back(definedEnt); };
	void addDefinedEnt(std::string definedEnt) { definedByEnts.push_back(definedEnt); };
	void addArg(std::string argType) { argTypes.push_back(argType); };
	static std::string getMethodDescKey(std::string methodName, const std::vector<DataType*>& args)
	{
		std::string ret = methodName+"(";

		for (size_t i = 0;i < args.size(); i++)
		{
			std::string argTypeName = std::string(args[i]->getName());
			ret += argTypeName + ",";
		}
		ret += ")";
		return ret; 
...
public:
	...
	virtual bool generateCommonMap(const EntityDef::SCRIPT_MODULES& modules);
	virtual bool writeEntityCommonClass() { return false; };
	virtual bool needClientMethod(MethodDescription* pMethodDescription);
...
protected:
	...
	std::string entityCommonBody_;
	std::map<std::string, CommonPropDesc>  propsMap_;
	std::map<std::string, CommonMethodDesc> methodsMap_;
};
