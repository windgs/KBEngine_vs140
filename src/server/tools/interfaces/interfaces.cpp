	...
	uint32 intaddr, uint16 intport, uint32 extaddr, uint16 extport, std::string& extaddrEx)
{
	...
	if (componentType == DBMGR_TYPE)
	{
		...
		//add by huyf 2019.07.16:interface进程onInterfaceAppReady修改
		// 所有脚本都加载完毕
		PyObject* pyResult = PyObject_CallMethod(getEntryScript().get(), const_cast<char*>("onInterfaceAppReady"), const_cast<char*>(""));

		if (pyResult != NULL)
			Py_DECREF(pyResult);
		else
			SCRIPT_ERROR_CHECK();
		//add end:interface进程onInterfaceAppReady修改
	}
